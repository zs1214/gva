...
func (b *SysBaseMenu) AddBaseMenu() (err error) {
	findOne := mysql.DEFAULTDB.Where("name = ?", b.Name).Find(&SysBaseMenu{}).Error
	if findOne != nil {
		b.NickName = b.Title
		err = mysql.DEFAULTDB.Create(b).Error
	} else {
...
func (b *SysBaseMenu) DeleteBaseMenu(id float64) (err error) {
	err = mysql.DEFAULTDB.Where("parent_id = ?", id).First(&SysBaseMenu{}).Error
	if err != nil {
		err = mysql.DEFAULTDB.Where("id = ?", id).Delete(&b).Error
		err = mysql.DEFAULTDB.Where("menu_id = ?", id).Unscoped().Delete(&SysMenu{}).Error
	} else {
...
func (b *SysBaseMenu) UpdataBaseMenu() (err error) {
	...
	err = mysql.DEFAULTDB.Where("id = ?", b.ID).Find(&SysBaseMenu{}).Updates(upDataMap).Error
	err1 := mysql.DEFAULTDB.Where("menu_id = ?", b.ID).Find(&[]SysMenu{}).Updates(upDataMap).Error
...
func (b *SysBaseMenu) GetBaseMenuById(id float64) (err error, menu SysBaseMenu) {
	err = mysql.DEFAULTDB.Where("id = ?", id).First(&menu).Error
	return
}
...
func (m *SysBaseMenu) GetBaseMenuTree() (err error, menus []SysBaseMenu) {
	err = mysql.DEFAULTDB.Where(" parent_id = ?", 0).Order("sort", true).Find(&menus).Error
...
func getBaseChildrenList(menu *SysBaseMenu) (err error) {
	err = mysql.DEFAULTDB.Where("parent_id = ?", menu.ID).Order("sort", true).Find(&menu.Children).Error
